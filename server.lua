...
if Config.useKVSkills then
    exports.KV_skills:registerSkill({
        name = 'gunsmithing',
        multiplier = 2.5,
        maxlevel = 30,
        maxReward = 20,
...
local function fixWeapon(payload)
    ...
    TriggerClientEvent('KV_weaponrepair:repairitem', payload.source, payload.toSlot.name)
end

RegisterNetEvent('KV_weaponrepair:startweaponrepair', function(data)
    local source = source
    ...
    if slot and slot.name == data.name then
        ...
        if not Config.locations[data.bench].free then
            ...
            if count >= requiredAmount then
                ...
                TriggerClientEvent('KV_weaponrepair:repairitem', source, data.name)
            else
...
RegisterNetEvent('KV_weaponrepair:fixweapon', function()
    local source = source
    if repairs[source] then
        ...
        if slot and slot.name == repairs[source].name then
            ...
            if Config.useKVSkills then
                exports.KV_skills:addXP(source, 'gunsmithing', Config.xpreward)
            end
            repairs[source] = nil
